...
sql="SELECT nom_client, prenom_client
FROM clients 
INNER JOIN commandes ON clients.num_client = commandes.num_client
ORDER BY nom_client;"

sql="SELECT clients.num_client, clients.nom_client, clients.prenom_client, clients.code_ville, clients.num_client, commandes.num_commande
FROM clients
JOIN commandes ON clients.num_client = commandes.num_client
ORDER BY nom_client;"

//-------->!!!! Distinc enleve les doublon mais faire attention car sa peut retirer deux client du meme nom !!!!
sql="SELECT DISTINCT clients.nom_client, clients.prenom_client 
FROM clients
INNER JOIN commandes ON clients.num_client = commandes.num_client 
WHERE YEAR(commandes.date_commande)=2023;"

sql="SELECT famille_article.nom_famille , COUNT(*) AS "nb article" //----> COUNT combient de famille_article
